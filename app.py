    ...
    if request.method == 'POST':
        ...
        try:
            conn.execute('''
                INSERT INTO users (username, password, full_name, class, role)
                VALUES (?, ?, ?, ?, 'student')
            ''', (username, hashed, full_name, student_class))
            conn.commit()
    ...
    if request.method == 'POST':
        ...
        conn.execute('''
            INSERT INTO analizler (
                ...
                sosyal_dogru, sosyal_yanlis, sosyal_bos,
                ingilizce_dogru, ingilizce_yanlis, ingilizce_bos,
                basarili_konular, zayif_konular, hedef_toplam, taahhut
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            session['user_id'],
            request.form.get('sinav_tarihi'),
            request.form.get('sinav_adi'),
            request.form.get('turkce_dogru', 0),
            request.form.get('turkce_yanlis', 0),
            request.form.get('turkce_bos', 0),
            request.form.get('matematik_dogru', 0),
            request.form.get('matematik_yanlis', 0),
            request.form.get('matematik_bos', 0),
            request.form.get('fen_dogru', 0),
            request.form.get('fen_yanlis', 0),
            request.form.get('fen_bos', 0),
            request.form.get('sosyal_dogru', 0),
            request.form.get('sosyal_yanlis', 0),
            request.form.get('sosyal_bos', 0),
            request.form.get('ingilizce_dogru', 0),
            request.form.get('ingilizce_yanlis', 0),
            request.form.get('ingilizce_bos', 0),
            request.form.get('basarili_konular'),
            request.form.get('zayif_konular'),
            request.form.get('hedef_toplam'),
            request.form.get('taahhut')
        ))
        conn.commit()
        conn.close()
